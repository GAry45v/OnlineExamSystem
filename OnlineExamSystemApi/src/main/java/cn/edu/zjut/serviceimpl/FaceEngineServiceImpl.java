...
import cn.edu.zjut.face_detect.factory.FaceEngineFactory;
...
public class FaceEngineServiceImpl implements FaceEngineService {
    ...
    @Override
    public List<FaceUserInfo> compareFaceFeature(byte[] faceFeature, String faceId) throws InterruptedException, ExecutionException {
        List<FaceUserInfo> resultFaceInfoList = Lists.newLinkedList();//识别到的人脸列表

        FaceFeature targetFaceFeature = new FaceFeature();
        targetFaceFeature.setFeatureData(faceFeature);
        List<FaceUserInfo> faceInfoList = userFaceInfoMapper.getUserFaceInfoByfaceId(faceId); //从数据库中取出人脸库
    ...
    @Override
    public boolean compareFace(byte[] inputBytes, String targetfaceId) {
        FaceEngine faceEngine = null;
        try {
            FaceFeature inputFeature=new FaceFeature(inputBytes);
            //获取引擎对象
            faceEngine = faceEngineObjectPool.borrowObject();
            List<FaceUserInfo> faceInfoList = userFaceInfoMapper.getUserFaceInfoByfaceId(targetfaceId); //从数据库中取出人脸库
            if (faceInfoList == null || faceInfoList.isEmpty()) {
                logger.warn("未找到与 faceId 对应的用户信息: {}", targetfaceId);
                return false;
            }
            for (FaceUserInfo faceUserInfo : faceInfoList) {
                // 将数据库中的特征转换为 FaceFeature
                FaceFeature targetFeature = new FaceFeature();
                targetFeature.setFeatureData(faceUserInfo.getFaceFeature());

                // 初始化相似度变量
                FaceSimilar faceSimilar = new FaceSimilar();

                // 调用引擎接口进行特征比对
                int compareResult = faceEngine.compareFaceFeature(inputFeature, targetFeature, faceSimilar);

                if (faceSimilar.getScore() >= 0.9) { // 假设 0.8 是匹配阈值
                    logger.info("人脸匹配成功，faceId: {}, 相似度: {}", targetfaceId, faceSimilar.getScore());
                    return true; // 匹配成功，直接返回 true
                }
            }
        } catch (Exception e) {
            logger.error("", e);
